# 🧮 Benvenuto/a nel Portale di Matematica & Fisica

In questo spazio troverai formulari riassuntivi, guide teorico-metodologiche, schede di laboratorio ed eserciziari graduati svolti passo-passo per gli studenti delle scuole superiori e dell'università.

---

## 🏫 Scuole Superiori

### 📐 Matematica
* 🔹 [**Primo Anno**](matematica/superiori/primo-anno/)  
  *Insiemi, calcolo letterale, equazioni lineari, frazioni algebriche e geometria euclidea nel piano.*
* 🔹 [**Secondo Anno**](matematica/superiori/secondo-anno/)  
  *Disequazioni lineari, sistemi, radicali, equazioni/disequazioni di 2° grado, piano cartesiano.*
* 🔹 [**Terzo Anno**](matematica/superiori/terzo-anno/)  
  *Equazioni e disequazioni irrazionali, valore assoluto, coniche (circonferenza, parabola, ellisse, iperbole).*
* 🔹 [**Quarto Anno**](matematica/superiori/quarto-anno/)  
  *Goniometria, trigonometria, esponenziali e logaritmi, calcolo combinatorio, probabilità e geometria solida.*
* 🔹 [**Quinto Anno**](matematica/superiori/quinto-anno/)  
  *Limiti, continuità, calcolo differenziale, studio completo di funzione e integrali.*

### ⚡ Fisica
* 🔸 [**Primo Anno**](fisica/superiori/primo-anno/)  
  *Grandezze fisiche, incertezze di misura, calcolo vettoriale, statica ed equilibrio dei corpi solidi.*
* 🔸 [**Secondo Anno**](fisica/superiori/secondo-anno/)  
  *Cinematica (1D e 2D), principi della dinamica, forze d'attrito, lavoro, potenza, energia e conservazione.*
* 🔸 [**Terzo Anno**](fisica/superiori/terzo-anno/)  
  *Quantità di moto, gravitazione universale, meccanica dei fluidi e termodinamica.*
* 🔸 [**Quarto Anno**](fisica/superiori/quarto-anno/)  
  *Fenomeni ondulatori, acustica, ottica geometrica/ondulatoria ed elettrostatica.*
* 🔸 [**Quinto Anno**](fisica/superiori/quinto-anno/)  
  *Corrente elettrica e circuiti, campi magnetici, induzione elettromagnetica, equazioni di Maxwell e cenni di relatività.*

---

## 🎓 Università

| Area | Corso / Modulo | Principali Argomenti Trattati |
| :--- | :--- | :--- |
| **Matematica** | 📐 [**Analisi Matematica 1**](matematica/universita/analisi-1/) | Numeri complessi, successioni, limiti, serie numeriche, derivate e integrali. |
| **Matematica** | 📐 [**Analisi Matematica 2**](matematica/universita/analisi-2/) | Calcolo differenziale multivariabile, integrali doppi/tripli, forme differenziali, ODE. |
| **Matematica** | 📊 [**Algebra Lineare**](matematica/universita/algebra-lineare/) | Vettori, matrici, spazi vettoriali, diagonalizzazione, autovalori e autovettori. |
| **Fisica** | ⚛️ [**Fisica Generale 1**](fisica/universita/fisica-1/) | Meccanica newtoniana, moti relativi, dinamica dei sistemi, termodinamica classica. |
| **Fisica** | 🧲 [**Fisica Generale 2**](fisica/universita/fisica-2/) | Elettrostatica, magnetostatica, correnti variabili, onde e ottica fisica. |

---

## 📩 Lezioni e Contatti

Hai bisogno di lezioni individuali, percorsi di recupero del debito formativo o preparazione a verifiche ed esami universitari?

* ✉️ **Email:** [matematica@elisa.land](mailto:matematica@elisa.land)
